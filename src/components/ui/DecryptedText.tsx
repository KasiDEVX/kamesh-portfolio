        ...
        if (direction === 'forward') {
          setRevealedIndices(prevRevealed => {
            if (prevRevealed.size < text.length) {
              ...
              // Schedule display update outside the updater to avoid side-effects in updater
              setTimeout(() => setDisplayText(shuffleText(text, newRevealed)), 0);
              if (newRevealed.size >= text.length) {
                clearInterval(intervalRef.current ?? undefined);
                setTimeout(() => { setIsAnimating(false); setIsDecrypted(true); }, 0);
              }
              return newRevealed;
            } else {
              clearInterval(intervalRef.current ?? undefined);
          ...
          setRevealedIndices(prevRevealed => {
            if (pointerRef.current < orderRef.current.length) {
              ...
              if (newRevealed.size === 0) {
                clearInterval(intervalRef.current ?? undefined);
                setTimeout(() => { setIsAnimating(false); setIsDecrypted(false); }, 0);
              }
              return newRevealed;
            } else {
              clearInterval(intervalRef.current ?? undefined);
